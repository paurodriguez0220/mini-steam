...
/** Lifts on hover of the enclosing card or hero, which carries `group`. */
const MASCOT =
  "[transform-box:fill-box] origin-center transition-transform duration-[520ms] ease-spring " +
  "group-hover:-translate-y-[9px] group-hover:scale-105 group-hover:-rotate-2";
...
function resolveScene(title: string): CoverScene {
  ...
  return (
    <g>
      ...
      <g className={MASCOT}>
        ...
        <path
          d="M236 108 h 16 m -16 6 l 18 8 m -18 -20 l 18 -8"
          stroke="#e8352e"
          strokeWidth={5}
          strokeLinecap="round"
          fill="none"
  ...
  return (
    <div
      className={`relative block aspect-4/3 w-full overflow-hidden rounded-lg ${
        isHero
          ? "[box-shadow:inset_0_0_0_3px_rgba(34,26,22,0.07),var(--shadow-soft-2)]"
          : "shadow-[inset_0_0_0_2px_rgba(34,26,22,0.07)]"
      }`}
    ...
    >
      ...
      <span
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(200deg,rgba(255,255,255,0.5)_0%,rgba(255,255,255,0)_42%)]"
        aria-hidden="true"
      />

      {ribbon ? (
        <span className="absolute bottom-3.5 left-3.5 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 font-display text-xs font-extrabold text-[#221a16] shadow-soft-1">
          {ribbon}
        </span>
      ) : null}
    </div>
